...
const MinimalistTemplate = ({ data }: TemplateProps) => {
    return (
        <div className="bg-white min-h-[1200px] p-12 font-sans" style={{ overflow: 'visible' }}>
                ...
                <div className="mb-10">
                    ...
                    <div className="space-y-3">
                        {data.skills.technical.length > 0 && (
                            <div>
                                <span className="text-xs text-gray-500 uppercase tracking-wider">Technical:</span>
                                <p className="text-sm text-gray-700 mt-1">{data.skills.technical.join(" • ")}</p>
                            </div>
                        )}
                        {data.skills.languages.length > 0 && (
                            <div>
                                <span className="text-xs text-gray-500 uppercase tracking-wider">Languages:</span>
                                <p className="text-sm text-gray-700 mt-1">{data.skills.languages.join(" • ")}</p>
                            </div>
                        )}
                    </div>
                </div>
                        ...
                        {Object.entries(data.codingProfiles || {}).map(([platform, url]) => {
                            ...
                            return (
                                <div key={platform} className="text-sm">
                                    <span className="text-gray-500">{platform.charAt(0).toUpperCase() + platform.slice(1)}:</span>
